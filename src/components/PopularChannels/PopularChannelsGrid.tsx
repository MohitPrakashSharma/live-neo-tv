...
export const PopularChannelsGrid = ({ onChannelSelect }: PopularChannelsGridProps) => {
  ...
  return (
    ...
    >
      ...
      <div 
        ref={scrollRef}
        className="grid grid-flow-col auto-cols-[160px] sm:auto-cols-[200px] md:auto-cols-[280px] lg:auto-cols-[320px] gap-2 md:gap-3 overflow-x-auto scrollbar-hide scroll-smooth pb-1"
      >
      ...
      <div className="hidden md:block">
        {canScrollLeft && (
          <button
            onClick={scrollLeft}
            className={`absolute left-0 top-1/2 translate-y-2 p-2 bg-black/50 rounded-r-lg transition-opacity ${
        ...
        {canScrollRight && (
          <button
            onClick={scrollRight}
            className={`absolute right-0 top-1/2 translate-y-2 p-2 bg-black/50 rounded-l-lg transition-opacity ${
              isHovering ? 'opacity-100' : 'opacity-0'
            }`}
